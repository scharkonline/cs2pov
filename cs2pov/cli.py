...
def cmd_comms(args) -> int:
    ...
    from .comms import apply_comms_overlay
    from .trim import get_video_duration, compute_keep_segments, load_transitions

    # Recompute keep segments for standalone comms on an already-trimmed video.
    # For tick-nav mode: use GotoTransitions sidecar for actual video durations
    # (includes polling overrun), avoiding cumulative drift.
    keep_segments = None
    if is_trimmed and timeline and timeline.alive_segments:
        if args.tick_nav:
            # Try loading transitions sidecar for actual segment durations.
            # The sidecar is saved as {output_stem}_transitions.json during recording,
            # where output_stem is the base name (e.g., "recording"). Try multiple
            # candidate paths since the user may pass the trimmed or raw video.
            try:
                transitions = None
                # Try: video_path itself, then strip _raw suffix, then add _raw suffix
                candidates = [video_path]
                stem = video_path.stem
                if stem.endswith("_raw"):
                    # User passed raw video — sidecar is named after the base stem
                    base = video_path.parent / f"{stem[:-4]}{video_path.suffix}"
                    candidates.insert(0, base)
                else:
                    # User passed trimmed video — sidecar might also be next to raw
                    raw_path = video_path.parent / f"{stem}_raw{video_path.suffix}"
                    candidates.append(raw_path)

                for candidate in candidates:
                    transitions = load_transitions(candidate)
                    if transitions:
                        break

                if transitions:
                    sorted_transitions = sorted(transitions, key=lambda t: t.pause_video_time)
                    num_alive = len(timeline.alive_segments)
                    num_trans = len(sorted_transitions)

                    # Build per-alive-segment durations from transition timestamps.
                    # transition[i] fires at end of alive_segment[i], so:
                    #   alive[i] video_dur = transition[i].pause - transition[i-1].unpause
                    # For alive[0]: no preceding transition (startup contaminates),
                    #   use demo-time duration as fallback (~0.25s overrun, acceptable).
                    per_segment_durations: list[tuple[float, float]] = []
                    for i in range(num_alive):
                        if i == 0 or i > num_trans or i - 1 >= num_trans:
                            # Fallback: demo-time duration with tick_nav extensions
                            seg = timeline.alive_segments[i]
                            dur = seg.end_time - seg.start_time
                            if seg.reason_ended == "death":
                                dur += 0.9
                            elif seg.reason_ended == "round_end":
                                next_seg = timeline.alive_segments[i + 1] if i + 1 < num_alive else None
                                if next_seg:
                                    for r in timeline.rounds:
                                        if r.round_num == next_seg.round_num and r.prestart_time is not None:
                                            dur = r.prestart_time - 0.2 - seg.start_time
                                            break
                            per_segment_durations.append((0.0, dur))
                        else:
                            dur = sorted_transitions[i].pause_video_time - sorted_transitions[i - 1].unpause_video_time
                            per_segment_durations.append((0.0, dur))

                    if per_segment_durations:
                        keep_segments = per_segment_durations
                        if args.verbose:
                            durations = [end - start for start, end in keep_segments]
                            print(f"  Using {len(keep_segments)} per-segment durations from transitions")
                            print(f"    Total duration: {sum(durations):.1f}s")
                            print(f"    Segment 0 (demo-time fallback): {durations[0]:.1f}s")
                            print(f"    Segments 1+  (transition-based): avg {sum(durations[1:])/max(len(durations)-1,1):.1f}s")
                elif args.verbose:
                    print("  No transitions sidecar found, falling back to tick-nav extension logic")
            except Exception as e:
                if args.verbose:
                    print(f"  Warning: Could not load transitions: {e}, falling back to tick-nav extension logic")
        else:
            try:
                video_duration = get_video_duration(video_path)
